...
pub fn inject_memories(mut req: ConversationRequest, memories: &[MemoryRecord]) -> ConversationRequest {
...
mod tests {
    ...
    use uuid::Uuid;
    use chrono::Utc;
    use vkdg_operations::{ConversationRequest, CapabilitySet};
    ...
    fn memories_prepended_to_existing_system() {
        let req = ConversationRequest { system: Some("You are helpful.".into()), ..empty_req() };
        let out = inject_memories(req, &[mem("User prefers Python")]);
        let sys = out.system.unwrap();
        assert!(sys.contains("User prefers Python"), "memory must be in system");
        assert!(sys.contains("You are helpful."), "original system must be preserved");
        assert!(sys.find("User prefers Python") < sys.find("You are helpful."),
