...
mod tests {
    ...
    fn healthy_ranks_above_degraded() {
        ...
        let good = CandidateSignals {
            connection_id: "good".into(),
            health: 1.0,
            quota_headroom: Some(0.8),
            cost_per_ktoken: Some(100),
            latency_p50_ms: Some(100),
            instability_events: 0,
        };
        let bad = CandidateSignals {
            connection_id: "bad".into(),
            health: 0.1,
            quota_headroom: Some(0.1),
            cost_per_ktoken: Some(9000),
            latency_p50_ms: Some(4000),
            instability_events: 10,
        };
    ...
    fn cost_saver_mode_prefers_cheap() {
        ...
        let cheap = CandidateSignals {
            connection_id: "cheap".into(),
            health: 0.9,
            quota_headroom: Some(0.5),
            cost_per_ktoken: Some(10),
            latency_p50_ms: Some(300),
            instability_events: 0,
        };
        let expensive = CandidateSignals {
            connection_id: "expensive".into(),
            health: 1.0,
            quota_headroom: Some(0.9),
            cost_per_ktoken: Some(5000),
        ...
        assert_eq!(
            ranked[0], "cheap",
            "cost-saver must prefer cheap connection"
        );
    }
    ...
    fn ship_fast_mode_prefers_low_latency() {
        ...
        let fast = CandidateSignals {
            connection_id: "fast".into(),
            health: 0.9,
            quota_headroom: Some(0.5),
            cost_per_ktoken: Some(1000),
        ...
        let slow = CandidateSignals {
            ...
            quota_headroom: Some(0.9),
            cost_per_ktoken: Some(10),
            latency_p50_ms: Some(3000),
            instability_events: 0,
        };
        let ranked = rank_candidates(&[slow, fast], &w);
        assert_eq!(
            ranked[0], "fast",
            "ship-fast must prefer low-latency connection"
        );
    }
    ...
    fn unknown_signals_treated_neutrally() {
        ...
        assert!(
            score > 0.0 && score < 1.0,
            "unknown signals must yield mid-range score, got {score}"
        );
    }
    ...
    fn mode_pack_names_map_correctly() {
        ...
        let weights: Vec<ScoringWeights> = modes
            .iter()
            .map(|m| ScoringWeights::from_mode_pack(m))
            .collect();
        assert!(
