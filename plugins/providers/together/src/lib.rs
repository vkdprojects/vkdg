//! VKDG provider plugin: Together AI (OpenAI-compatible endpoint).
...
impl ProviderAdapter for TogetherProvider {
    fn id(&self) -> &str {
        "together"
    }
...
fn build_body(req: &ConversationRequest, config: &ConnectionConfig) -> Bytes {
    let model = config
        ...
        .first()
        .cloned()
        .unwrap_or_else(|| "meta-llama/Llama-3.3-70B-Instruct-Turbo".into());
